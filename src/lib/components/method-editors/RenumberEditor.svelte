...
<div class="space-y-2 text-xs">
	...
	<div class="flex items-center gap-2">
		<label for="renumber-start" class="w-12 opacity-50 shrink-0">起始</label>
		<input
			id="renumber-start"
			type="number"
			min="0"
	...
	<!-- 步长 -->
	<div class="flex items-center gap-2">
		<label for="renumber-step" class="w-12 opacity-50 shrink-0">步长</label>
		<input
			id="renumber-step"
			type="number"
			min="1"
	...
	<!-- 填充位数 -->
	<div class="flex items-center gap-2">
		<label for="renumber-padding" class="w-12 opacity-50 shrink-0">位数</label>
		<input
			id="renumber-padding"
			type="number"
			min="1"
		...
		/>
		<span class="opacity-30">位补零</span>
	</div>
			...
			{#each POSITION_OPTIONS as opt}
				<button
					class="px-2 py-0.5 rounded text-[10px] transition-colors
						{config.position === opt.value ? 'bg-blue-500/30 text-blue-300' : 'bg-surface-500/10 opacity-50 hover:opacity-70'}"
	...
	<!-- 分隔符 -->
	<div class="flex items-center gap-2">
		<label for="renumber-separator" class="w-12 opacity-50 shrink-0">分隔</label>
		<input
	...
	<div class="px-2 py-1.5 rounded bg-surface-500/5 border border-surface-500/10">
		<p class="opacity-40 text-[10px]">
			预览: <span class="text-blue-400/70 adr-mono">{previewNumber}</span>
		</p>
	</div>
</div>
