...
<div class="space-y-2 text-xs">
	...
	<div class="flex items-center gap-2">
		<label for="move-from" class="w-12 opacity-50 shrink-0">起始</label>
		<input
			id="move-from"
			type="number"
			min="0"
		...
		/>
		<span class="opacity-30">位置</span>
	</div>

	<!-- 字符数量 -->
	<div class="flex items-center gap-2">
		<label for="move-count" class="w-12 opacity-50 shrink-0">数量</label>
		<input
			id="move-count"
			type="number"
			min="1"
		...
		/>
		<span class="opacity-30">个字符</span>
	</div>

	<!-- 目标位置 -->
	<div class="flex items-center gap-2">
		<label for="move-to" class="w-12 opacity-50 shrink-0">移至</label>
		<input
			id="move-to"
			type="number"
			min="0"
	...
	<div class="px-2 py-1.5 rounded bg-surface-500/5 border border-surface-500/10">
		<p class="opacity-40 text-[10px]">
			将第 <span class="text-blue-400/70 adr-mono">{config.fromStart}</span>~<span class="text-blue-400/70 adr-mono">{config.fromStart + config.count - 1}</span> 位的
			<span class="text-blue-400/70 adr-mono">{config.count}</span> 个字符移至第 <span class="text-blue-400/70 adr-mono">{config.toPosition}</span> 位
		</p>
	</div>
</div>
