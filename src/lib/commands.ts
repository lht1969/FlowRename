...
/** 扫描目录并加载文件列表 */
export async function scanDirectory(request: ScanRequest): Promise<ScanResponse> {
	return invoke<ScanResponse>('scan_directory', { request });
}

/** 扫描选中的文件列表 */
export async function scanFiles(request: ScanFilesRequest): Promise<ScanResponse> {
...
/** 预览重命名操作结果 */
export async function previewRename(request: PreviewRequest): Promise<PreviewResponse> {
	return invoke<PreviewResponse>('preview_rename', { request });
}

/** 执行实际文件重命名 */
export async function executeRename(request: ExecuteRenameRequest): Promise<ExecuteResponse> {
