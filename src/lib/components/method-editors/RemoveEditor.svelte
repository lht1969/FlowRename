...
<div class="space-y-2 text-xs">
	...
	<div class="flex items-center gap-2">
		<label for="remove-count" class="w-12 opacity-50 shrink-0">数量</label>
		<input
			id="remove-count"
			type="number"
			min="1"
	...
	<div class="flex items-center gap-2">
		...
		<div class="flex gap-1">
			<button
				class="px-2 py-0.5 rounded text-[10px] transition-colors {config.position === 'Start' ? 'bg-blue-500/30 text-blue-300' : 'bg-surface-500/10 opacity-50 hover:opacity-70'}"
				onclick={() => update({ position: 'Start' })}
			>开头</button>
			<button
				class="px-2 py-0.5 rounded text-[10px] transition-colors {config.position === 'End' ? 'bg-blue-500/30 text-blue-300' : 'bg-surface-500/10 opacity-50 hover:opacity-70'}"
